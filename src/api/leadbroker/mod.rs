...
fn require_lead_broker(role: &str) -> Result<(), E> {
    if role == "Lead Broker" {
        Ok(())
    } else {
...
async fn assert_lb_owns_project(pool: &PgPool, user_id: Uuid, project_id: Uuid) -> Result<(), E> {
...
/// PATCH /me/lb/lots/{id} — Available ↔ Reserved only (keeps pricing fields).
pub async fn patch_lot_reserve(
    Extension(pool): Extension<PgPool>,
    headers: HeaderMap,
    Path(id): Path<Uuid>,
    Json(p): Json<LbReserveLotInput>,
    ...
    if p.status != "Available" && p.status != "Reserved" {
        return Err((
            ...
            "Lead Broker may only set Available or Reserved",
        ));
    }
    ...
    assert_lb_owns_project(&pool, user_id, lot.project_id).await?;
...
/// GET /me/lb/projects/{id}/contracts
...
fn lead_broker_id_from_agents(agents: &Value) -> Option<String> {
    agents.as_array()?.iter().find_map(|a| {
        if a.get("role").and_then(|v| v.as_str()) == Some("lead-broker") {
            a.get("id")
                .and_then(|v| v.as_str())
                .map(|s| s.to_string())
        } else {
...
fn collect_direct_and_downline_seller_ids(agents: &Value, fallback_root: Option<&str>) -> HashSet<String> {
    let root = lead_broker_id_from_agents(agents)
...
async fn load_project_agents(
    ...
    let row = sqlx::query(
        "SELECT agents_json, lead_broker_roster_id FROM public.projects WHERE id = $1",
    )
    .bind(project_id)
    .fetch_optional(pool)
    .await
...
fn resolve_lb_subject_id(agents: &Value, lead_broker_roster_id: Option<Uuid>) -> Option<String> {
...
pub async fn list_project_payments(
    ...
    headers: HeaderMap,
    Path(project_id): Path<Uuid>,
) -> Result<Json<Vec<LbCashFlowPaymentResponse>>, E> {
    let (user_id, role) = require_session_user(&pool, &headers).await?;
    require_lead_broker(&role)?;
    assert_lb_owns_project(&pool, user_id, project_id).await?;
