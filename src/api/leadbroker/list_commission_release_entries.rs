...
/// GET /me/lb/projects/{id}/commission-release-entries — LB subject only, read-only.
pub async fn list_commission_release_entries(
    ...
    let subject_id = assert_lb_owns_project(&pool, user_id, project_id).await?.to_string();
