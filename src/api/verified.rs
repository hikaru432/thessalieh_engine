...
/// Like Verified but also checks the pubkey is registered in user_keys. Used for all other write endpoints.
pub struct VerifiedStrict(pub Vec<u8>, pub Vec<u8>);
...
impl<S: Send + Sync> FromRequest<S> for VerifiedStrict {
    type Rejection = (StatusCode, &'static str);
    ...
    async fn from_request(req: Request<Body>, state: &S) -> Result<Self, Self::Rejection> {
        let pool = req
            ...
            .get::<PgPool>()
            .cloned()
            .ok_or((StatusCode::INTERNAL_SERVER_ERROR, "Missing db pool"))?;

        let (msg, pubkey) = extract_verified(req, state).await?;

        let exists: bool =
            sqlx::query_scalar("SELECT EXISTS(SELECT 1 FROM public.user_keys WHERE pubkey = $1)")
                .bind(&pubkey)
                .fetch_one(&pool)
                .await
                .map_err(|e| {
        ...
        if !exists {
            return Err((StatusCode::UNAUTHORIZED, "Unregistered key"));
        }

        Ok(VerifiedStrict(msg, pubkey))
    }
}
