...
/// GET /me/upline/{role_slug}/projects/{project_id}/contract-split-history — read-only,
/// whole project (a contract's split history isn't scoped to any one subject, same as
/// admin's version; this subject already sees every contract in the project via
/// list_project_contracts). Needed so this subject's own commission schedule can be
/// recomputed the same way admin's is — without this, a mid-stream split change
/// silently isn't reflected here.
